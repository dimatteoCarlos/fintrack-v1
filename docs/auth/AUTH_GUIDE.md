...
| ----------------- | ------------- | --------------------------------------------------- |
| `token_id`        | `UUID`        | **Primary key**, defaulted from `gen_random_uuid()`  |
| `user_id`         | `UUID`        | Foreign key to `users`, `ON DELETE CASCADE`          |
| `token`           | `TEXT UNIQUE` | **The SHA-256 hex digest of the JWT, never the JWT** |
| `expiration_date` | `TIMESTAMPTZ` | When the row stops being accepted                    |
| `revoked`         | `BOOLEAN`     | Set on logout, on rotation and on password change    |
| `user_agent`      | `TEXT`        | Audit metadata                                       |
| `ip_address`      | `TEXT`        | Audit metadata                                       |
| `created_at`      | `TIMESTAMPTZ` | Issue time                                           |
| `updated_at`      | `TIMESTAMPTZ` | Last rotation                                        |

**The table stores a digest, not a credential.** `hashToken` in `authFn.js` computes
`sha256(token)` as hex, and the refresh endpoint hashes what the cookie presents and looks the
digest up. A reader of the table cannot mint a session from it.

**SHA-256 and not bcrypt**, deliberately: the input is a signed JWT with full entropy, not a
human password, and the digest is computed on every refresh. Bcrypt's slowness defends against
guessing a low-entropy secret, which is not the threat here, and would tax every request.
...
Ownership check for a route that addresses an account (`targetAccountId` or `id`).
It authenticates first when `req.user` is missing, then applies the rule below.
...
The centralized authentication controller of the frontend, and the single place
that decides what happens to an identity. It reaches the API through `authFetch`
and publishes to `useAuthStore`, so a component never calls an auth endpoint
itself.
...
| Method            | Description                                                                          |
| ----------------- | ------------------------------------------------------------------------------------ |
| `handleSignIn`    | Authenticates credentials, sets the token expiry and redirects to the initial page    |
| `handleSignUp`    | Registers an account and opens its session in the same call                           |
| `handleSignOut`   | Invalidates the session on the server, then runs `logoutCleanup` locally              |
| `checkAuthStatus` | Validates the stored access token on mount; this is what restores a session on reload |

### Internal utilities

- **`mapUserResponseToUserData`**: Maps the backend naming of a user row, `user_firstname` and its siblings, onto the camelCase shape the components are typed against. One place converts, so no component reads a snake_case key.
- **`extractErrorMessage`**: Turns a failure into one sentence a person can read, splitting three cases — a server answer with a status to interpret (401 invalid credentials, 429 rate limited, 400 validation), an unreachable server, and an unexpected runtime error.
...
const LoginView = () => {
 ...
 const onLogin = async (credentials) => {
  ...
  if (success) {
   // the hook has already navigated
  }
 };
...
| --- | --- |
| [Auth state indicators](AUTH_TABLES.md) | Every state indicator of the system, table by table and store by store, with the matrix of what each scenario produces |
| [AuthEvent architecture](AUTH_EVENT_ARCHITECTURE.md) | How a navigation event such as a password change or an expired session is handled declaratively: a handler returns a description and `AuthPage` executes it |
| [Future auth events](FUTURE_AUTH_EVENTS.md) | Worked examples of adding an event, for extensions not in the codebase |
| [Utilities reference](AUTH_UTILS.md) | The functions behind the flows above, including `authFn.js` and `authStorage.ts` |
