...
| R4 | **1-space indentation** in every file, CSS included. | Project-wide standard. |
| R5 | **Comments and identifiers in English.** | Project-wide standard. |
| R6 | **No decorative rules.** No `/* ===== */` banners, no emoji section headers. A comment states *why*, in one or two lines. | A banner hides the code; the reason is what the comment is for. |
