...
| Phase | State |
|---|---|
| Schema + write path | Landed (`3b72371`, `adc1150`) |
| Backend read path | Landed (`efad4c3`, `0e6cb1a`) |
| Frontend | Landed for the Budget board, the month editor and the variance screen, which call the four routes; other screens still read the dashboard endpoint |
| Closure / legacy retirement | Not started |
...
broken number is still the one on screen. The migration of §17.4 is where that
changes.
...
This is an architectural invariant, **not** a
...
place and `createTables.js` / `ensureBudgetTables()` create
...
planned in the superseded-code cleanup (§26). See F-09.
...
Collected from the SQL constraints and the guards in
...
```
 UI (not built yet — the edit modal)
...
wrote. That constraint is the V2 boundary.
...
|---|---|---|---|
| Account creation | `createAllocationForAccount` → `insertFirstAllocation` | The **account's start month**, not the current one | No — the creation form has no checkbox |
...
which is what renders the helper text: *"From September it goes back to $300."*
...
| Constant | Value | Note |
|---|---|---|
| `AMOUNT_SCALE` | 2 | Becomes per-currency in a later change |
...
```
 ...
 Called by:   the account card (BudgetCard) and the layout totals (BudgetLayout)
...
two differ on every request that named a past month.
...
```
 ...
 Called by:   the read-only history (Insights)
...
re-implemented in five places. All are removed or rewired by the migration of §17.4.

### 17.3 Frontend commit sequence

| Commit | Files | What lands |
|---|---|---|
| `feat(budget): define budget contract types` | `types/budgetTypes.ts` | The 145 current lines are discarded whole and replaced by the shapes of §15. Nothing imports the file today, so it is a zero-risk rewrite |
| `feat(budget): add budget api client` | new `fintrack/services/` | Four typed functions over the four routes, plus **one** error handler for the §15.5 envelope |
| `feat(budget): add budget edit modal` | new component | Amount field + the `onlyThisMonth` checkbox (unchecked). Helper text is rendered **from `restoresTo` / `restoresFrom`**, never computed |
| `feat(budget): show current month status` | account card | The three render states of I-3, and the second line shown only when `nextMonthBudget !== budgetAmount` |
| `refactor(budget): read budget from module` | `BudgetLayout.tsx:15`, `ListCategory.tsx:44,61,85,93` | **The A → C migration** |
| `feat(budget): add read-only history` | Overview / Insights | Consumes `/series`; renders `months[]` directly and `totals` as the header |

### 17.4 The A → C migration in detail
...
`ListAccountOfCategory.tsx:49`. Deciding
which screen shows the per-account card is a layout decision of the account-card commit; no budget
rule depends on it.
...
| | | `getTotalSpentByAccountAndPeriod`, `getTransactionsByAccountAndPeriod` | **nobody** (§25) | — | — |
...
**Frontend, target (after the sequence of §17.3)**
...
```
 ...
 all four through → fintrack/services/budgetApi  (the api client of §17.3)
```
...
no code deletes an allocation row (F-07). Deleting the account cascades the
...
budget it is compared against is August's.
...
| Editing an exception | `carried` = amount in force at M | ✔ | §12 | `writeAllocation:119` |
...
| Rate limiting | none on the mount (`app.js:144`) | A single API-wide counter treated a dashboard paint like a password guess, so it is left off |
...
| Read by | the whole frontend | nobody | nobody yet — the frontend commits of §17.3 |
...
allocation service, inside the same transaction. That is what makes the A → C
migration a read-path switch rather than a data migration.

### Classification of every legacy artifact

| Artifact | Status | Evidence |
|---|---|---|
| `category_budget_accounts.budget` | **MIGRATION** | Still written and still read by the frontend. Dropped when the legacy budget column is retired (§26), after an inventory of its readers |
...
| `budgetAllocationRepository.getAllocationBefore` | **V2** | Uncalled since `adc1150`. Kept deliberately: a `< M` lookup is what authoring a future month would need |
| `budgetTransactionRepository.getTotalSpentByAccountAndPeriod`, `…getTransactionsByAccountAndPeriod` | **DEAD** | Uncalled |
| `budgetAllocationRepository.deleteAllocationsForAccount` | **REMOVED** | Had no caller anywhere and is not in this tree. Going back to unbudgeted (§21, scenario 6) has no endpoint |
| `periodResolver.js` (197), `getNumberOfPeriods.js` (27) | **DEAD** | Deleted in the superseded-code cleanup (§26) |
| `budgetPolicyService.js`, `budgetVsActualCalculator.js`, `core/budgetPolicy.js`, `core/budgetAllocation.js` | **DEAD** | **Note:** none of these files exist on disk any more — the current tree has only the nine files listed in §18 |
| `calculateBudgetMetrics` (`getAccountController.js:41`) | **MIGRATION** | Reads `cba.budget`; retired with the legacy budget column (§26) |
| `dashboardController.js:185-186, 347` | **MIGRATION** | The defective subtraction; retired with the legacy budget column (§26) |
| `frontend/types/budgetTypes.ts` | **DEAD** | Describes frequencies and SCD2; **zero importers**, verified |
| `frontend/…/categoryBudgetCalculations.ts` | **MIGRATION** | Becomes redundant at the A → C migration (§17.4) |
...
```
 3b72371      refactor(budget): replace schema and write path
              ...
              │
 adc1150      fix(budget): resolve carried at the month
              │ getAllocationBefore (< M) → getAllocationForMonth (<= M),
              │ read BEFORE the UPSERT.
              │
 efad4c3      refactor(budget): rebuild the budget api
              │ 6 routes → 4, period resolution removed,
              │ PUT /accounts/:id/current, POST /accounts/status
              │
 0e6cb1a      feat(budget): add month series endpoint
              │ GET /accounts/:id/series, /export rewritten on the same
              │ service, assertBudgetFigures + makeBudgetMonthStatus extracted
              │
 ─────────── HEAD (feat/budget) ───────────
              │
              feat(budget): define budget contract types      ← next
              feat(budget): add budget api client
              feat(budget): add budget edit modal
              feat(budget): show current month status
              refactor(budget): read budget from module        ← A → C
              feat(budget): add read-only history
              inventory of cba.budget readers
              chore(budget): retire legacy budget column
              chore(budget): remove superseded budget code
```
...
and belong to the audit that led to it.
...
| The response contract | §15 — **frozen**, and the frontend is written against it, not against what the server happens to return |
| What the user currently sees on screen | still `category_budget_accounts.budget`, until the A → C migration (§17.4) |
...
| A response field | the factory, `budgetCalculationService`, `exportUtils`, `budgetTypes.ts`, §15 | all four endpoints | **High** — §15 must be amended, not bypassed |
...
| The account card | the account-card component | none | Low |
| Dropping `cba.budget` | `dashboardController`, `getAccountController`, both account controllers, five frontend files | the legacy read path | **High** — requires the A → C migration (§17.4) first |
...
| **F-03** | Account creation returns `budget_allocation` with 4 fields where the PUT returns 7, and §15 covers only the PUT | `accountCategoryCreationcontroller.js:424`, repository `:213,:163` | The creation shape is uncovered | §15 | Medium | Open — narrowed: the account PATCH no longer returns the key |
| **F-04** | Four nullable sites exist: `executionPercentage`, `subcategory`, `restoresTo`, and the whole `totals` block under mixed currencies | `makeBudgetAccountStatus.js:29`, repository `:233`, `:171`, `budgetCalculationService.js:229` | A client type that assumes a single nullable field is wrong | §15 | Medium | Open |
| **F-05** | `makeTotals([])` returns `currency: null` with **numeric** amounts, so `currency === null` is not a valid mixed-currency discriminator. Unreachable via HTTP (`accountIds.min(1)`), reachable in-process | `budgetCalculationService.js:226,229` | A client discriminating on `currency` mis-renders | §15.1 | Low | Open |
| **F-07** | No code deletes an allocation row: the repository function written for it had no caller and is not in this tree. The removal verb *"this account is not budgeted"* has no endpoint | grep over `backend/src` | Scenario 6 is unreachable | none | Medium | Open |
| **F-08** | The local `migrations` table records `012_backfill_budget_policies.sql`, the **pre-rename** name. `012_backfill_budget_allocations.sql` has never been applied here | live query | The local DB does not match the chain; a `db:migrate` would run the new file now | none | Low | Open (expected: the file was renamed in place) |
| **F-09** | `budget_frequency_types`, `budget_policies` and `budget_policy_allocations` still exist physically. No current DDL creates them and **no migration drops them** | live query; grep over `db/` | A fresh DB and this one have different schemas | none | Medium | Open (planned: `chore(budget): remove superseded budget code`) |
| **F-10** | `getAccountController.js:31-33` claims the budget endpoints *"price from `budget_policy_allocations`, which carry a frequency and a validity range"* | that comment | False since `efad4c3` | none | Low | Open |
| **F-11** | `frontend/types/budgetTypes.ts` still declares frequencies, SCD2 `validFrom`/`validUntil` and retired endpoints. **Zero importers** | grep over `frontend/src` | The only written FE contract describes a model that no longer exists | §15 | Medium | Open (planned: `feat(budget): define budget contract types`) |
...
| **F-13** | `budgetConfig.js` survives for one reader that is itself uncalled | `initDatabase.js:42,57` | A config with no live consumer | none | Low | Open (§25) |
| **F-14** | The CSV omits unbudgeted months by design; a consumer counting rows per month will see gaps | `exportUtils.js:75` | Deliberate, and undocumented outside the code comment | §15.4 | Low | Open |
...
|---|---|
| No documented shape for the account creation/edit `budget_allocation` field | §15 |
| No documented way to un-budget an account (delete its rows) | an endpoint |
| No drop migration for the three legacy tables, with UP and DOWN | `chore(budget): remove superseded budget code` |
| No stated verification procedure for a frontend commit, given F-15 | the first frontend commit (`feat(budget): define budget contract types`) |
...
**1. Where do I start?**
§15 for the contract, then `budgetCalculationService.js` for
...
| A field | the factory, the service, `exportUtils.js`, **and §15** |
...
|---|---|
| Boot | start the backend, require `Server running on port 5000` |
| Backend behaviour | one-off scripts against the live service: force a multi-month history on one account, assert the series against direct `SELECT`s, then restore the rows |
...
```
 ...
 FINDINGS:                 13  (1 high, 5 medium, 7 low)
