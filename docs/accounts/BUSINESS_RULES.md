...
| :--- | :--- | :--- |
| `bank`, `investment`, `category_budget` | Must be **0 or positive**. | `deposit` |
...
> **A savings pocket is not in this matrix and cannot be.** A pocket is a savings
> plan, not an account: committing money to one writes a row in
> `pocket_allocations` and moves nothing, so the funding bank keeps its balance.
> Migration `020_create_pocket_tables.sql` replaced the `pocket_saving` account
> type that used to take part in transfers. See **Savings Pockets** in the root
> `README.md`.
...
*   **Invalid Flow ❌**: `CATEGORY_BUDGET` → `BANK` (Violates Category Budget lock)
