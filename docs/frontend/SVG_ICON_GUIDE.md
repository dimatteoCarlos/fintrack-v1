...
`frontend/src/assets/accountingDashboardSvg/` — seven files, imported live by
`editionAndDeletion/utils/accountTypeIcons.ts:8-14`.

**The color half of this is resolved.** No file hardcodes a stroke: all seven
carry `stroke="currentColor"` and take their color from the class passed at the
call site.

**Three still declare a fixed size on the root**, so they will not scale with
`font-size` the way the budget control bar's set does:

| File | Root size |
| :--- | :--- |
| `bankAccountSvg.svg` | `32 × 32` |
| `cashAccountsSvg.svg` | `26 × 16` |
| `incomeAccountsSvg.svg` | `24 × 18` |

The other four — `debtsAccountsSvg.svg`, `expenseAccountsSvg.svg`,
`investmentAccountsSvg.svg` and `pocketsAccountsSvg.svg` — declare none and
already meet the checklist.
