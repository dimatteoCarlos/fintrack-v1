// Contract tests for the level-2 analysis sections (pure builders, no database). Parts cannot
// disagree with their whole; absent means "no statement was run", empty means "the owner has
// none", and a withheld ratio is null, never 0.
...
test('an absent level asks for no analysis at all', () => {
 // The default: a client that has not been updated receives the pre-level-2 payload.
...
test('the derived debt level publishes nothing and says why', () => {
 // The net series is already fetched and would be free, but a net position that has
 // not moved hides both legs doubling, which this level exists to stop.
...
test('the floored shortfall is what the per-account floor absorbed', () => {
 // One account holds 100 against 300 committed, another 900 against nothing: free
 // cash is 900, not 700, since a surplus cannot cover another account's shortfall.
 // The gap is published because a client subtraction would be wrong for this owner.
 const analysis = makePocketAnalysis({
  level: ANALYSIS_FULL,
  months,
  pockets,
