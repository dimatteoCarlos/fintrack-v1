// Contract tests for the reporting window: the server reports the window it used, so a running month
// stops at the reference date and a closed one at its own last day. No database: the builder is pure
// and takes the clock as an argument.
...
test('a day past the served month is clamped to that month', () => {
 // Covers the two clocks disagreeing: the database still calls September current
 // while the process has rolled into October. A reference date outside the served
 // month would be a period no figure was cut at.
