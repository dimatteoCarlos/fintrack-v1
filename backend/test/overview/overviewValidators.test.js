// Contract tests for the Overview request schemas. Every schema is strict on purpose:
// a retired parameter answers 400 naming the key instead of being stripped silently,
// a property no response shows and only a test can hold.
...
test('the page endpoint takes no analysis level', () => {
 // The page has no paginated list and no level-2 section: it overviews every
 // domain, so a depth parameter would request six analyses at once.
