// Checks getRelatedAccounts (the close screen's related accounts) in a rolled-back transaction on fabricated
// accounts. Run from backend/: node scripts/verifyRelatedAccounts.js [--expect fintrack_dev]
// Refuses to run unless the database is the --expect one, on loopback and unencrypted (as verifyClose.js).
...
try {
 ...
 // A target and two counterparties. The names carry the script's name so a row that
 // outlives the rollback is identifiable on sight.
 ...
 // A movement recorded against the target, naming a counterparty on the other side.
 // account_id is always the target: the shared CTE reads only the target's own signed rows,
 // so a row owned by the counterparty is invisible to this query by construction.
 ...
 // The fund: one movement, so its breakdown has one entry and the cell names the
 // movement without repeating the count.
 await record('investment', -25.0, fundId);

 // Neither of these may reach the list: the first is the target's own opening row, which
 // names no counterparty; the second is incomplete, and the CTE filters status='complete'.
 await client.query(
  `INSERT INTO transactions(
     user_id, description, amount, movement_type_id, transaction_type_id,
 ...
 // The breakdown is the same rows grouped deeper; if this stops holding, the cell shows a
 // total its own parts contradict.
 ...
 // Catalog names, not labels: the browser dictionary does the naming, and a pre-translating
 // server would make language a property of the response.
 check(
