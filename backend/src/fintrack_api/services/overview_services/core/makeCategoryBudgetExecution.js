// The execution rate the expense Pareto prints beside its chart: categorized spending over the
// budget of the same categories. It is not the card's execution, which divides TOTAL spend and so
// includes the uncategorized part.

import { money, toAmount, toRate } from '../../budget_services/core/money.js';

/**
 * Reads the month's execution off the ranked breakdown's last running figures, so the rate and
 * the chart head cannot disagree.
 *
