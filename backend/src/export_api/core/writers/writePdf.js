// Renders the period-statement dataset from statementReportService.js as an A4 PDF with pdfkit.
// No financial logic: it only formats, lays out and charts figures already computed there;
// charts are pdfkit vector primitives, not a charting library.
...
// Literal hex copies of frontend/src/styles/tokens.css: pdfkit cannot read CSS variables.
const PALETTE = {
 ink: '#141414',
 secondary: '#404040',
 onDark: '#ffffff',
 ...
 // Pocket levels recalibrated: the on-screen values (#5faa78, #3db87a, #60b1d6) give white text <4.5:1.
 // Ahead is teal, not a second green (only 6.2 from statusComplete in CIE Lab; teal is 21.0 from its
 // nearest neighbour, info), so the two badges stay distinct at pill size.
 statusComplete: '#1f6f43',
 statusAhead: '#1f6f6f',
 // Lighter than secondary; the one entry with no tokens.css counterpart, because
 // #404040 prints too heavy under a long ranking of bars.
 rankingBar: '#6b6b6b',
};
...
// Figures must reserve the width of a closing parenthesis so decimals align; pdfkit
// has no ::after, so the callers' column widths carry that.
...
// executionPercentage already arrives 0-100, unlike the 0-1 ratios fmtRate formats;
// scaling it again printed "7070.0%" for a 70.7% category.
...
// An uncomputable figure prints an em dash plus a footnote mark, never 0. Marks are
// letters, not numbers: "(1)" inside a cell of figures reads as part of the amount.
// Past the 26th note the letters double (aa, ab).
...
function drawTable(doc, { x, y, width, columns, rows, compact = false }) {
 ...
 rows.forEach((row) => {
  ...
  columns.forEach((col, i) => {
   ...
   // Badge: the status word in white on a pill of its status colour. The word stays
   // the primary signal and the colour only repeats it.
...
// Section 9's execution ring: the executed arc in its status colour over a light track.
function drawExecutionDonut(doc, { x, y, width, categoryExecution }) {
 ...
 if (categoryExecution && categoryExecution.executionPercentage !== null) {
  // executionColorFor and the ring both expect a 0-1 ratio; the served figure is
  // 0-100, so it is scaled once here.
 ...
 // Starts level with the ring, not the block's top edge, so the panel sits beside the donut.
 const infoX = x + 124;
 let infoY = y + 30;
 ...
 // Must clear the ring's bottom edge (y + 106) and the panel's last hairline (y + 119).
 let legendY = y + 124;
 const legend = [
...
// A rectangle filled with a 45-degree hatch and an outline. Section 4's bar pair differs
// by texture, not only tone, so it survives grayscale print and photocopies. The open step
// avoids a solid block of ink; lines are clipped to the rect so a one-point-wide bar stays clean.
...
// Geometry shared by the monthly net-flow chart (bars plus a haloed line) and the debt
// trend chart (lines only).
function drawSeriesChart(doc, { x, y, width, height, months, mode }) {
 ...
 // Anchored at plotX, not the chart's outer edge, so the axis-number gutter does not
 // separate the legend from its grid.
 const legendY = y + 2;
 if (mode === 'debt') {
  ...
  // Hatched income vs ochre expenses: two lightnesses of one ink read as the same bar twice.
 ...
 if (mode === 'flow') {
  ...
  months.forEach((m, i) => {
   const cx = xAt(i);
   // Same hatch and fill as the legend swatches.
...
// Sources named individually before the tail folds into one part. Seven plus the fold
// equals the eight colours of SOURCE_COLORS, so no two parts of the ring share a hue.
const SOURCE_PARTS = 7;

// Legend name, with the same fallback order as the income screen: unattributed,
// closed account with no name, closed account with its name, plain name.
...
// Section 5's income ring, one arc per source; drawn only for several sources, since a single
// 100% arc repeats the table. Shares come from the server and are never recomputed, so an arc's
// angle cannot disagree with its printed percentage.
...
// Ranked bar table (sections 6 and 7): one bar per row, the row crossing 80% cumulative in bold.
// labelOf/labelHeading let the subcategory ranking reuse it. rest (a tail the caller cut) is drawn
// without a bar: the folded total would be the longest and compress every real row.
function drawRankedCategoryChart(
 doc,
 {
  ...
  labelWidth = 86,
  // Keep each label on one line, cut with an ellipsis. Off by default: category names
  // fit the column, subcategory labels (which carry the nature) often do not.
  clipLabel = false,
  rest = null,
 },
) {
 ...
 // A wrapped label would overprint the rows below at the 14pt row pitch. pdfkit's own
 // `lineBreak: false, ellipsis: true` did not cut here, so the cut uses widthOfString.
 ...
 if (rest) {
  ...
  // Cumulative cell left empty on purpose: the fold is 100% by construction and the
  // Total line below already prints it.
  cy += 14;
 }
...
// Section 9's Pareto: spent bars (filled) beside budget bars (outline), two cumulative lines and the 80%
// mark. Every legend text() sets fillColor first: a swatch's .fill()/.stroke() leaves its ink as the
// current fill colour, which a following text() would inherit instead of PALETTE.ink.
function drawParetoChart(doc, { x, y, width, height, rows }) {
 ...
 // Anchored at plotX, not x, so it lines up with the grid instead of the tick labels.
 // Each item's x follows the previous item's measured text width, so the row never
 // overlaps itself whatever paretoW is.
 const legendY = y;
 const GAP = 6;
 let lx = plotX;
 ...
 // Value labels on the spending line only; the budget line carries none.
...
// Budget accounts listed before the rest folds into one row. With the fold and the total
// that is 27 lines at 14pt (about 378), leaving room for the heading and note on an A4 page.
const SUBCATEGORY_ROWS = 25;

// The nature is part of the label: two accounts sharing a subcategory keep separate rows
// and the nature tells them apart.
const subcategoryLabel = (row) =>
 row.nature ? `${row.subcategoryName} (${row.nature})` : row.subcategoryName;

// Cuts text to a column width using the font currently set on doc; the caller sets
// font and size first.
...
// The footer sits inside the bottom margin, and pdfkit's .text() auto-paginates any call
// past that boundary regardless of x/y, so the margin is zeroed for this call and restored.
...
// The debt and pocket pages are skipped when their data is empty, so the page count is
// known only after rendering; bufferPages keeps every page addressable so each footer
// can carry the real total.
...
function renderDocument(doc, data, { generatedAt, timeZone }) {
 const {
  ...
  financialGoals,
  categories,
  // Defaulted so a caller without these fields degrades (no subcategory page, no income
  // ring, empty year-to-date columns) instead of throwing.
  subcategories = [],
  incomeBySource = [],
  categoryYearToDate = [],
  categoryExecution,
 ...
 // Footers are drawn in a second pass once the page count is known, so this draws
 // only the header.
 ...
 // Its height depends on the owner's account count (one row each), so it is measured
 // first (heading 15, header band 14, 13.5 per row as drawTable lays down) and moved to
 // its own page when it does not fit; page breaks are manual and nothing reflows.
 ...
 // Every heading qualifier states its currency so the figures read unambiguously alone.
 ...
 // Sits under the monthly table because the ring breaks down that table's income line.
 ...
 // Section 7, the category ranking one level down, gets its own page: its rows would run
 // past the foot of the categories' page and page breaks are manual. One row is one
 // budget account (category / subcategory / nature); nothing is summed into it.
 if (subcategories.length > 0) {
  ...
  // The ranking's own total, not the month's: these rows are budget accounts only, so
  // dividing by the month's expense would understate every share while the Total line
  // still printed 100%.
  const subcategoryTotal = lastSubcategory.cumulativeActual ?? 0;
  // Over the whole ranking, not the drawn part: a crossing past the cut is stated in
  ...
  y = drawRankedCategoryChart(doc, {
   ...
   // Wider than the category column and cut, not wrapped: each label carries its nature
   // in brackets and at 86 points a third of them wrapped into the rows below.
   labelWidth: 150,
   clipLabel: true,
   rest:
 ...
 if (categories.length === 0) {
  ...
  // Year-to-date spend per category from the single SQL-grouped query, keyed on the
  // lowercase category_name both sides carry.
  ...
  // A category can be absent from the year-to-date rows when its account was closed
  // during the year and took its category_name with it.
 ...
 // The Pareto chart already prints "No categorized expense this month." when categories
 // is empty, so the table is skipped rather than repeating it.
 if (categories.length > 0) {
 ...
 // pdfkit auto-paginates any .text() crossing the bottom margin, splitting the goals table or notes
 // mid-row onto an unheaded page. The block is bounded (3-row table, at most 3 footnotes, 3
 // KNOWN_LIMITS), so 210pt always suffices; otherwise start a fresh page.
 ...
 // Second pass: every page exists, so each footer can state the real total.
