...
--   survive the deletion of the user_accounts row.
--
-- WHY THIS FILE LIVES IN THE MIGRATION CHAIN. The registry is not documentation:
--   it is a structural dependency of the schema, so its DDL belongs with the
--   other migrations.
--
-- WHAT THE REGISTRY IS: not a table of closed accounts, but the durable
--   historical identity of an account, which lets references survive when
--   user_accounts loses the row. Every account has a row here from the moment
--   it is created, written by the trigger below, and that row carries a null
--   closed_at for as long as the account is open. Closure fills the closure
--   columns; it does not create the row.
--
-- 035 WAS THE NEXT FREE NUMBER when this file was written: 034 was the highest
--   on every branch.
--
...
-- this repository and can be read. An earlier commit edited an applied
-- migration, and the cleanup ran for weeks: fintrack_dev holds three tables
-- the current text of 010 does not create, and four separate places in the
-- codebase describe that situation wrongly - two boot files, budgetConfig.js
-- and getAccountController.js. A migration edited after it ran makes a database's
...
--
-- THE NAME. "Registry" was already the working name in the design, which
-- described the shape as "a registry of every account id ever issued". Two
-- alternatives were considered and both were worse.
