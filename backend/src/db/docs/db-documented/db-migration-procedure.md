...
Neither stands: a read-only connection to production on 2026-09-03
read the ledger and found **29 rows** — the
...
wrote its ledger row while creating nothing.
...
- **The single transaction is not as single as it looks.** Migrations `001`-`007`
 ...
 so a failure in file N+1 leaves N already committed.
