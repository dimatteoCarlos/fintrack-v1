// The auth panel as a dialog; own component because a hook cannot run inside AuthPage's `showModal &&`
// branch (unmounting also returns focus). Portalled into document.body: useModalDialog sets `inert`
// on #root, which would disable a dialog left inside it.
...
function AuthModal({ onClose, isDarkTheme, ...authUIProps }: AuthModalPropsType) {
 ...
 // Backdrop and Escape, the accidental exits, land here; the Close button is
 // deliberate and closes at once. An empty form asks too, with its own message.
 ...
 const { titleId, dialogProps } = useModalDialog({
  ...
  // The first field, not the panel, which would leave the header's theme toggle
  // one Tab from the caret. Queried, not named: the first field differs between
  // sign-in and sign-up.
  onInitialFocus: (panel) => panel.querySelector('input')?.focus(),
 });

 return createPortal(
  // The backdrop closes through the unsaved-changes confirm. The panel stops the
