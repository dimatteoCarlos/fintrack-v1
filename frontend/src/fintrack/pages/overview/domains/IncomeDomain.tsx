...
// the month's income came from, which waits for the full answer.
...
// One label ladder for the figure above the list and the list itself, so the same
// account is not named differently in the headline and in its row.
   ...
   <AnalysisPanel
    ...
    figure={
     <p className='domainAnalysis__figure'>
      ...
      {/* Source is named, not described: bySource is sorted, so bySource[0] is the largest. Unnamed
          until the full answer lands, the only state where the share has no row behind it. */}
     ...
     {sources?.map((source) => (
      ...
      <li className='domainAnalysis__item' key={source.accountId ?? 'unattributed'}>
       <LevelThreeRow
        ...
        // A closed account keeps its name and loses the link: this is a period
        // breakdown, and the destination shows the account's current state, not
        // what the figure beside it describes.
        link={
