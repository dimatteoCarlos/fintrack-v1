// Expense on level 2: series, spending against budget by category, and the list's category filter.
...
// The server narrows only the transaction page, so categories[] keeps every category while one is
// selected. The top six by rank are chips; the rest fold behind More (34 categories pushed Movements down).
...
const CategoryFilter = ({
 ...
 const titleId = useId();
 // Handed to the category screen so its back arrow returns here. Pathname only:
 // that screen re-adds the month from its own URL, and carrying it on both ends
 // would leave two month parameters in one address.
 ...
 // Null when nothing is picked, the same condition the link renders under; built
 // once so `to` and `state` cannot come from two different calls.
 const detailLink =
  selected === null ? null : categoryLink(selected, month, pathname);

 const topNames = new Set(
  categories.slice(0, VISIBLE_CATEGORIES).map((c) => c.categoryName),
 );
 // Pinned first only when it would otherwise be folded behind More; one already
 // among the visible six keeps its rank position, so picking a chip does not
 // shift the strip.
 const pinned =
 ...
 return (
  <section className='categoryFilter' aria-labelledby={titleId}>
   ...
   >
    {/* Present even when nothing is selected, and pressed then: a clear control
        that appears only after a click hides the way back until the reader has
        committed. */}
    <button
     type='button'
   ...
   {selected !== null && detailLink !== null && (
    <Link
     ...
     viewTransition
    >
     {/* The category is its own element so it can carry the subject colour this
         screen gives it; the name is the only part that changes. */}
     {'Open '}
...
// Bars drawn before the tail folds into one. A fixed count, not the 80% line (that would pin the
// reference line to the last bar every month) and not viewport-based (the fold must not vary by screen).
...
// The nature is part of the name, not a second axis: an account is the triple
// category / subcategory / nature, so this tells apart two accounts that share a
// subcategory.
...
/**
 * The flat ranking's bars: the first twenty accounts, then everything else in one. The fold carries
 * the last row's running figures (100%), so the curves cover every account while the bars do not.
 */
...
function ExpenseDomain({
 ...
 // The level the chart ranks at while no category is selected. Inside a category
 // it already ranks accounts, so the switch is not offered: a second control
 // would contradict the one selection that governs chips, chart and list.
 ...
 const trail = (
  <div className='budgetPareto__trail'>
   {/* The category name is the subject of the drilled figure, so it carries the title voice;
       at the top level there is no subject and the row is a caption for the whole set. */}
   ...
   {/* Last in the row and in the DOM, so reading order and visual order agree
       without CSS reordering; the stylesheet pushes it to the far edge with an
       auto inline-start margin. */}
   {isNarrowed && (
    <button
     type='button'
    ...
    >
     <ArrowLeftSolidSvg
      ...
      aria-hidden='true'
     />
     {/* Names what it will show, not "Back": clearing returns to whichever
         ranking the switch is on, and a reader who arrived by chip never saw a
         column to go back to. */}
 ...
 return (
  <>
   ...
   {/* The fold replaces the chart rather than expanding under it: opening a
       column already replaces the ranking, so one gesture has one consequence. */}
   {isFoldOpen ? (
    <section className='domainCards domainCards--single'>
     <figure className='budgetPareto'>
      ...
      <div className='budgetPareto__head'>
       {/* The fold's own money and its share of the month: the question is
           whether a tail spread over many lines is worth acting on, which an
           execution rate over unrelated accounts does not answer. */}
