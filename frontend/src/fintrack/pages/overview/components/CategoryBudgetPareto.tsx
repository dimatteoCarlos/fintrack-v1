// Level 2 expense: month Pareto of spent vs budget per row, plus the served running shares.
// Rows arrive pre-labelled and share one ranking; the `openable` flag tells group rows from single accounts.
...
// How far a cut bar's zigzag rises above the axis top, and how wide one tooth is.
// Only a folded row is ever cut.
...
type CategoryBudgetParetoProps = {
 ...
 title: string;
 // The drilled-into name, kept out of `title` so it can take the subject colour;
 // `title` also builds the chart's aria-label below, which cannot take a node.
 titleSubject?: string;
...
function CategoryBudgetPareto({
 ...
 // The ranking's single tab stop; arrow keys move between openable columns so a
 // long ranking does not become dozens of tab stops.
    ...
    >
     <section className='domainCards domainCards--single'>
      <figure className='budgetPareto'>
       ...
       {/* Fills its box; scrolls only when the rows need more room. */}
       <div
        ref={scrollRef}
       ...
       >
        <svg
         ...
         // A group when a column can be opened: role='img' hides its descendants
         // from assistive technology, so an inner control would be pointer-only.
