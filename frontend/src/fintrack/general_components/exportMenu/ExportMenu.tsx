// Saves the list on screen as a file in the format the reader picks; shared by budget, pocket and debts.
...
// The two formats every module list offers. The extension is in the label
// because the reader will look for the file by that name.
...
type ExportMenuPropType = {
 // The caller's request, already carrying the month or range on screen so the
 // file matches the list; the menu itself knows no endpoint.
 onExport: (format: ExportFormat) => Promise<void>;
 // What the file holds, for the accessible name; a bare "Export" repeated on
 // three screens names none of them.
 subject: string;
 // The surface the control sits on, not the colour it paints. 'seam' is the line
 // between the white header and the dark board (budget and pocket month
 // controls); 'dark' is the app ground a card sits on.
 surface?: 'seam' | 'dark';
 // Held off until the scope the request needs is on screen; same prop as MonthPicker.
 disabled?: boolean;
};
